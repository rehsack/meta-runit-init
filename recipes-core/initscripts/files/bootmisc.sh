...
then
  if [ "$(basename "${0}")" = "run" ]; then
     # For runit-init we don't need to write a file, we just use
     # the doneall service to detect when all booting is complete.
     [ "$VERBOSE" = "yes" ] && echo "System bootup in progress - please wait"
  else
